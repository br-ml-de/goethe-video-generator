...
class AudioGenerator:
    ...
    def generate_all_audio(self, production_json_path):
        ...
        # DEBUG: Check for intro/outro files
        intro_source = Path('assets/standard-audio/intro.mp3')
        outro_source = Path('assets/standard-audio/outro.mp3')
        print(f"🔍 DEBUG: Looking for intro at {intro_source.absolute()}")
        print(f"🔍 DEBUG: Intro exists: {intro_source.exists()}")
        print(f"🔍 DEBUG: Looking for outro at {outro_source.absolute()}")  
        print(f"🔍 DEBUG: Outro exists: {outro_source.exists()}")
